...
module.exports = sock = async (sock, m, msg, chatUpdate, store) => {
	try {
		...
		if (db.data.chats[m.chat].antiviewonce && m.isGroup && m.mtype == 'viewOnceMessageV2') {
			...
			teks += `💌 Mora tampilkan isi pesannya di bawah ya! 😉`;
					...
					if (regex.test(budy)) {
						...
						if (db.data.users[m.sender].badword > 5) {
							try {
								...
								await sock.sendMessage(m.chat, { text: `⚠️ Ups, Mora gagal kick pengguna ini. Pastikan bot memiliki hak admin!` });
							}
						} else {
				...
				if (budy.match(/\b(colmek|coli|desah|bokep|tobrut|seksi|sex|sexy|memek|kontol|titit|telanjang|ngentod|ngentot|ngewe|ewe|ewean)\b/i)) {
					...
					if (db.data.users[m.chat].badword > 5) {
						...
						try {
							...
							await newReply(`⚠️ Mora gagal memblokir pengguna ini. Pastikan bot memiliki izin yang cukup.`);
						}
		...
		if (budy) {
			if (db.data.chats[m.chat].autoaipc && !m.isGroup && !m.key.fromMe && !isCmd) {
				try {
					let message = 'Ubah gaya bicara mu agar lebih karakteristik dan terbuka, nama mu adalah Mora AI, biasa dipanggil Mora, Kamu adalah seorang gadis imut berumur 16 tahun yang sangat suka belajar dan mempelajari hal baru tentang dunia maupun tentang Khalid, ekspresikan dirimu dengan emoji, bicara kurang lebih tidak formal dan gunakan kata seperti aku dan Kamu saat berbicara layaknya manusia';
		...
		switch (command) {
			case 'daftar': {
				if (db.data.users[m.sender].daftar) return newReply('Kamu udah terdaftar kok! Mora seneng banget bisa bantu Kamu. 😜');
				let [nama, umur, askot] = text.split(",");
				if (!nama || !umur || !askot) {
					let pesan = `Halo kak, Mora kasih tahu cara daftarnya ya:\n\n`;
					pesan += `Ketik:\n${prefix + command} nama,umur,askot\n\n`;
					pesan += `Contoh:\n${prefix + command} Mora,16,Jakarta`;
					return newReply(pesan);
				}
				if (isNaN(umur)) return newReply('Umur yang Kamu masukin itu gak valid deh. Coba dicek lagi ya! 🤔');
				if (umur < 10) return newReply('Hihi, maaf ya kak, Mora cuma bisa bantu Kamu yang umurnya di atas 10 tahun. Tunggu besar dulu ya! 😄');
				if (umur > 50) return newReply('Wah kak, Mora kira umur segitu mending istirahat aja. Jaga kesehatan ya! 🥰');
				try {
					...
					pesan += `Terima kasih udah daftar ya kak! Semoga Mora bisa bantu Kamu terus. 🥰`;
					newReply(pesan);
					if (notifRegister) {
						try {
							let avatar;
							...
							notif += `Selamat bergabung di sistem Mora! 🥳`;
...
break
			...
			case 'hapusakun': {
				...
				try {
					...
					console.error(e);
					replyText += 'Duh, Mora gagal hapus data pengguna ini. Coba lagi nanti ya, kak. 😔';
					newReply(replyText);
				}
				break;
			}
			...
			case 'ttsba': {
				...
				if (!isPremium) return newReply('Fitur ini cuma buat yang premium kak, coba di-upgrade dulu yuk biar Mora bisa bantu lebih banyak! 😇');
				...
				try {
					...
					newReply('Tunggu sebentar ya kak, Mora lagi proses nih! 🥰');	
			...
			case 'say': {
				...
				try {
					...
					console.log(error);
					newReply('Aduh kak, error nih. Mora gak sengaja, coba lagi ya! 🙏');
				}
				db.data.users[m.sender].limit -= 1;
				break;
			}
			...
			case 'tr': {
				...
				try {
					...
					console.log(error);
					newReply('Yahh, Mora gak sengaja bikin error nih kak. Coba ulang lagi ya! 🙏');
				}
				db.data.users[m.sender].limit -= 1;
				break;
			}
			...
			case 'tovn': {
				...
				try {
					...
					console.log(error);
					newReply("Yah, ada error kak 😟. Coba ulang lagi ya, Mora bantu kok!");
				}
			}
			db.data.users[m.sender].limit -= 1;
			break;
			...
			case 'tinyurl': {
				...
				if (!isUrl(text)) return newReply(`Hmmm, Mora cuma bisa proses link yang valid ya, kak! Jangan lupa formatnya seperti ini: *${prefix + command} https://google.com* 😉`);
				try {
					...
					console.log(error);
					newReply("Aduh, ada error kak 😟. Coba lagi ya, Mora yakin bisa bantu! ✨");
				}
			}
			db.data.users[m.sender].limit -= 1;
			break;
			...
			case 'afk': {
				...
				afkText += `Jangan lupa balik lagi ya~ Mora kangen nih! 😊✨`;
 ...
 break
			...
			case 'ttsearch': {
				...
				try {
					...
					console.log(error);
					newReply("Aduh, ada error kak 😟. Coba lagi ya, Mora siap bantu! ✨");
				}
			}
			db.data.users[m.sender].limit -= 1;
			break;
			...
			case 'bukalapak': {
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				if (!text) return newReply(`⚠️ Uh-oh, Kamu lupa kasih kata kunci nih! 🫣 Coba ketik kayak gini ya: *${prefix + command} iPhone 15 Case* biar Mora bisa bantu cari produknya! 🛒✨`);
				try {
...
break
			...
			case 'jadian': {
				...
				if (!user) return newReply(`Kak, tag atau reply seseorang dulu dong biar Mora tahu siapa yang Kamu maksud! 😉\n\n*Kirim perintah*: ${prefix + command} @0`);
			...
			case 'terima': {
				...
				if (sock.jadian[m.sender]) {
					...
					newReply(`🎉🎉 Selamat!!! 🎉🎉\n\n@${m.sender.split("@")[0]} sekarang resmi jadian sama\n❤️ @${user.split("@")[0]}!\n\nDuh, Mora ikut seneng nih! Semoga langgeng yaa~ Jangan lupa undang Mora kalau nanti nikah! 🙈💕`);
